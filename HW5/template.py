...
class graph:
    ...
    def find_node_by(self, level_no, hmm_no, state_no):
        this_level = list(filter(lambda _:_.identifier[0] == level_no, self.template_nodes))
        this_hmm = list(filter(lambda _:_.identifier[1] == hmm_no, this_level))
        this_state = list(filter(lambda _:_.identifier[2] == state_no, this_hmm))
        return this_state
...
class template_node:
    def __init__(self, level_no, HMM_no, state_no , time=-1  , emitting = False):
        if(level_no == 0 and emitting == True):
            self.parents  = []
        if(state_no == 0):
            self.parents = t_graph.find_node_by(level_no, -1, 1)
        self.mu = []
        self.sigma = []
...
def main():
    ...
    for i in range(0, NO_OF_LEVELS):
        t_graph.add_node(template_node(i, -1, 1, emitting=True))
        for j in range(0, NO_OF_HMM):
            for k in range(0, NO_OF_STATES):
                t_graph.add_node(template_node(i, j, k))

    t_graph.add_node(template_node(2, -1, 1, emitting=True))
    pass
    t_graph.find_node_by(0,1,1)
