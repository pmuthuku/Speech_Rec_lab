    ...
    for j in range(2,n+1):
        ...
        for l in range(0,len(llist)-1):
            ...
            for i in range(2,min(2+k,mc)):
                ...
                dcostc[i,j]=min(dcostc[i,j-1],dcostc[i-1,j-1],dcostc[i-2,j-1])+dmat[i-2,j-1]
                pp=i
...
def main(argv):
     ...
     if argv[1]=='-d':
        ...
        #print '\n'
        costs=dtwtsyn(temdat,inpdat,llist)
        #print costs
