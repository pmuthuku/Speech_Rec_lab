...
mu_list = [np.zeros((NO_OF_STATES, 39)) for x in range(NO_OF_HMM)]# no of hmm has beeb set as 1
...
class template_node:
    def __init__(self, level_no, HMM_no, state_no, seq, time=-1, non_emitting=False):
        if (time == 0):
            ...
            self.parents = t_graph.find_node_by([level_no - 1], range(NO_OF_HMM), [NO_OF_STATES - 1], time_seq=time)#filetering will work 
        elif (state_no == 0):
...
def train_hmm(mapped_symbs, filenm,filewr):
    ...
    while curr_nod.identifier[3] != 0:
        ...
        statein = np.append(statein, curr_nod.identifier[2])
    ...
    boundary2 = np.where(result != -1)[0]
    timein = timein[boundary]
    timein = np.insert(timein,0,-1)
    #print timein
    statein =statein[boundary2]
    segment_list = []
    state_list = []
    filewr=filewr+'/'
    for i in range(len(mapped_symbs)):
        segment = data[timein[i]+1:timein[i+1]+1, :]
        stlist = statein[timein[i]+1:timein[i+1]+1]
        #print '{0}-----{1}----{2}----{3}--{4}'.format(segment.shape,mapped_symbs[i],timein[i]+1,timein[i+1]+1,stlist.shape)
        segment_list.append(segment)
        state_list.append(stlist)
        #print stlist
        cdigit=mapped_symbs[i]
        #segf=open(mapped_symbs[i]+'.frames','w')
        #np.savetxt(segf,segment)
        for j in range(5):
            fd=open(filewr+cdigit+'_'+str(j)+'.frames','a')
            st_id=np.where(stlist==j)[0]
            #print st_id
            if len(st_id) > 0:
                if len(st_id) == 1:
                    frame_write = np.matrix(segment[st_id[0],:])
                    np.savetxt(fd,frame_write)
                else:
                    st_start=st_id[0]
                    st_end=st_id[len(st_id)-1]
                    frame_write = np.matrix(segment[st_start:st_end+1,:])
                    np.savetxt(fd,frame_write)
...
def train_cont_hmms(transcrps, dirname,filewr):
    ...
    print fullname
    train_hmm(mapped_symbs,fullname,filewr)

if __name__ == '__main__':
    # train_cont_hmms(sys.argv[1],sys.argv[2])
    train_cont_hmms(sys.argv[1],sys.argv[2],sys.argv[3])
