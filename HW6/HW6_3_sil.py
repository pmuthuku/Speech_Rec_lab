...
def calculate_P(parents, identifier, C):
    ...
    for par_idx, parent in enumerate(parents):
        ...
        if  my_hmm_no == -1 and my_level_no == 1:# or parent_hmm_no==-1: # if parent is non-emitting then transition probability log is 0 but check this as insta. trans is from emitt to \non-emmiting. Non-emiitting to emmiting is computed on next frame
            trans_cost = -1.0*np.log(0.5)
        elif my_state_no ==0 and my_hmm_no ==10:
            trans_cost = -1.0*np.log(0.5)
        elif my_hmm_no ==-1 and my_level_no == 0:
            trans_cost = -1*np.log(0.00000000000001)
        elif parent_hmm_no == -1:
            trans_cost=-1*np.log(0.1)
        else:
            trans_cost= trans_list[my_hmm_no][parent_state_no, my_state_no]

        tr_cost = parent.P + trans_cost
        if tr_cost < best_P_prev:# > if +log and < if -log
            best_P_prev = tr_cost
            path_idx = par_idx
...
def main(file_name='cont_recordings/5_4.mfcc', file_number = 0):
    global t_graph
    t_graph = graph()
    global input_seq
        ...
        for i in range(0, NO_OF_LEVELS):
            ...
            for j in range(0, NO_OF_HMM):
                for k in range(0, NO_OF_STATES):
                    ...
                    kk=kk+1
            for k in range(0,NO_OF_STATES):
                t_graph.add_node(template_node(i,10,k,time=t))
    ...
    #first_frame=t_graph.template_nodes[3]
    #for nod in first_frame:
    #    print '{0}--->{1}--->{2}--->{3}'.format(nod.best_parent.identifier,nod.identifier,nod.P,nod.C)
